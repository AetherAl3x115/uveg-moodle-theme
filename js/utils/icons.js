 ...
 *   element.innerHTML = hi('dashboard');          // 20×20 por defecto
 *   element.innerHTML = hi('dashboard', 16);      // tamaño custom
 *   element.innerHTML = hi('dashboard', 20, 'my-class'); // con clase extra
 *
 * Reglas:
 *   - currentColor en stroke → hereda el color del CSS padre, siempre
 *   - stroke-width="1.5" estándar Heroicons
 *   - aria-hidden="true" siempre — accesibilidad la pone el elemento padre
 *   - Nunca hardcodear colores aquí
 * ─────────────────────────────────────────────────────────────
 */

/** Mapa clave → paths SVG (solo el interior del <svg>) */
const PATHS = {
  // ── Navegación principal ────────────────────────────────────

  // ti-layout-dashboard → squares-2x2
  ...
  // ti-message-circle → chat-bubble-oval-left
  ...
  // ti-arrow-bar-left → arrow-left-start-on-rectangle (colapsar sidebar)
  ...
  // ti-mood-empty → no existe neutro en Heroicons → face-smile con variante
  // usamos el mismo face-smile; la diferencia visual la da el color (ámbar)
  ...
  // ti-books → book-open (misma que modules, alias)
  ...
  // ti-book → book-open (alias de modules)
  ...
  // ti-circle-dot → signal (indicador de semana activa)
  ...
  // ti-layout-grid → squares-2x2 (alias de dashboard)
  ...
  // ti-info-circle → information-circle
  ...
  // Avatar card lección completada → clipboard-document-list
  ...
  // flag — propósito general / objetivo
  ...
  // academic-cap — competencias / educación
  ...
  // puzzle-piece — competencias específicas
  ...
  // exclamation-triangle — advertencia / políticas
 ...
 *
 * @param {string} key       - Clave del mapa PATHS (p.ej. 'dashboard')
...
export function hi(key, size = 20, extraClass = "") {
  ...
  if (!paths) {
    // Fallback: cuadrado vacío para detectar claves faltantes en dev
...
/**
 * Lista todas las claves disponibles (útil para debug en consola).
 * window.__uvegIcons() desde DevTools
 */
